(ns leiningen.new.org.sandbox.javamixed
  (:require
...
(defn org.sandbox.javamixed
  "A Leiningen template for mixed Java project (choices skeleton: skeleton_java).
  
usage: [LEIN_JVM_OPTS='-Dskeleton=skeleton_java'] lein new org.sandbox.javamixed [groupId/]artifactId
LEIN_JVM_OPTS='-DdataFile=data.yaml -DdataFmt=yaml -DffiLib=jna' lein new org.sandbox.javamixed org.sandbox/intro_jvm.util"
  [raw-name]
  (let [skel-dir (format "org/sandbox/javamixed/%s" (System/getProperty "skeleton" "skeleton_java"))
    ...
    (main/info "Generating fresh 'lein new' org.sandbox.javamixed project.")
